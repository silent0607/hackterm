  ...
  const handleConnect = async () => {
    ...
    try {
      await fetch('/api/vpn/connect', {
        ...
        body: JSON.stringify({ config: selectedFile, termId }) // Pass termId to see logs in terminal
      });
      fetchStatus();
    } catch (e) {}
  };
    ...
    <div style={{ paddingBottom: 40 }}>
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        <div>
          ...
          <div className={`info-panel ${vpnStatus.active ? 'border-green' : 'border-red'}`} style={{ padding: 20, textAlign: 'center', height: '100%' }}>
            {vpnStatus.active ? (
            ...
            <div style={{ marginTop: 16 }}>
              {vpnStatus.active ? (
                <button className="btn-pro btn-red" onClick={handleDisconnect} style={{ width: '100%' }}>{t('vpn_disconnect')}</button>
              ) : (
                <button className="btn-pro btn-green" onClick={handleConnect} disabled={!selectedFile} style={{ width: '100%' }}>{t('vpn_connect')}</button>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="info-panel" style={{ padding: 20 }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <label className="btn-pro btn-cyan" style={{ width: '100%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
